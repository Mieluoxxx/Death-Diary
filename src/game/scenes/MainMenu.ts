...
export class MainMenu extends Scene
{
    ...
    create ()
    {
        ...
        else if (this.textures.exists('menu') && this.textures.get('menu').has('top_logo_zh.png'))
        {
        ...
        {
            this.add
                .text(bgCenterX, 160, '死亡日记', { fontFamily: UI_FONT_FAMILY, resolution: UI_TEXT_RESOLUTION, fontSize: '42px', color: '#f0e6d2' })
                .setOrigin(0.5);
        }
        ...
        this.continueBtn = this.placeBigWhite(bgCenterX, btn2Y, t('continue', lan), canContinue, () =>
        {
    ...
    private setMenuButtonLabel (
        button: AtlasButton | GameObjects.Text | null,
        label: string,
    ): void
    {
        if (!button)
    ...
    {
        const has = this.textures.exists('ui') && this.textures.get('ui').has('btn_big_white_normal.png');
        ...
        const text = this.add
            .text(x, y, label, { fontFamily: UI_FONT_FAMILY, resolution: UI_TEXT_RESOLUTION, fontSize: '28px', color: enabled ? '#111' : '#666' })
            .setOrigin(0.5);
        if (enabled)
        {
