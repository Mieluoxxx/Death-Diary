...
export class ChooseScene extends Scene
{
    ...
    private buildRoleSlider (centerX: number, centerY: number, lan: LangCode): void
    {
        ...
        const frameW = this.textures.exists('ui') && this.textures.get('ui').has('frame_role_choose.png')
        ...
        if (sideMaskW > 0)
        {
            root.add(
                this.add
                    .rectangle(-viewWidth / 2 + sideMaskW / 2, 0, sideMaskW, viewHeight, 0x000000, 200 / 255)
                    .setOrigin(0.5),
            );
            root.add(
                this.add
        ...
        for (let i = 0; i < roleCount; i++)
        {
            ...
            if (this.textures.exists('ui') && this.textures.get('ui').has('page_view_indicator_1.png'))
            {
    ...
    private buildRolePortrait (slide: RoleSlide): GameObjects.Container
    {
        ...
        if (!slide.unlocked && this.textures.exists('icon') && this.textures.get('icon').has('icon_iap_lock.png'))
        {
            const lock = this.add
                .image(headerBg.width * 0.5 - 12, -headerBg.height * 0.5 + 12, 'icon', 'icon_iap_lock.png')
                .setScale(0.45)
                .setOrigin(1, 0);
            cell.add(lock);
        }
    ...
    private applyRoleIndex (index: number, animate: boolean): void
    {
        ...
        this.indicatorDots.forEach((dot, i) =>
        {
            ...
            const isActiveLayer = this.textures.exists('ui')
            ...
            else if (!this.textures.exists('ui') || !this.textures.get('ui').has('page_view_indicator_2.png'))
            {
    ...
    ): void
    {
        ...
        TALENT_IDS.forEach((talentId, index) =>
        {
            ...
            {
                cocosX = widthPadding2 + ((index - 3) % 2) * (widthPadding2 + nodeWidth) + nodeWidth / 2;
    ...
    ): TalentNode
    {
        ...
        {
            root.add(this.add.image(0, 5, 'icon', 'icon_iap_bg.png').setScale(0.9).setAlpha(opacity));
        }
        else if (this.textures.exists('ui') && this.textures.get('ui').has('frame_iap_bg_talent.png'))
        {
            root.add(this.add.image(0, 5, 'ui', 'frame_iap_bg_talent.png').setScale(0.55).setAlpha(opacity));
        }
        ...
        if (!unlocked && this.textures.exists('icon') && this.textures.get('icon').has('icon_iap_lock.png'))
        {
    ...
    private showLockedTalentDialog (lan: LangCode): void
    {
        ...
        else if (this.textures.exists('ui') && this.textures.get('ui').has('dialog_small_bg.png'))
        {
        ...
        {
            if (this.textures.exists('ui') && this.textures.get('ui').has('btn_common_black_normal.png'))
            {
